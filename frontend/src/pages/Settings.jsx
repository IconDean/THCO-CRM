...
import { settingsAPI, usersAPI, activityAPI, authAPI } from "../lib/api";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [webhooksData, usersData, activitiesData] = await Promise.all([
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-white/5 hover:bg-transparent">
                  ...
                  <TableHead className="text-[#8B8AA0] font-mono text-xs uppercase">Units</TableHead>
                ...
                {users.map((user) => (
                  <TableRow key={user.user_id} className="border-white/5 hover:bg-white/5">
                    ...
                    <TableCell className="text-[#8B8AA0]">
                      {user.role === "super_admin" ? "All" : (user.accessible_units?.length || 0)}
                    </TableCell>
                    ...
                    <TableCell>
                      <div className="flex gap-2">
                        <Button
                          ...
                          className="text-[#8B8AA0] hover:text-white hover:bg-white/5"
                        ...
                        <Button
                          ...
                          className={`${user.status === "active" ? "text-[#F87171] hover:text-[#F87171]" : "text-[#34D399] hover:text-[#34D399]"} hover:bg-white/5`}
                          disabled={user.user_id === currentUser?.user_id}
                        ...
                        <Button
                          ...
                          className="text-[#F87171] hover:text-[#F87171] hover:bg-white/5"
                          disabled={user.user_id === currentUser?.user_id}
