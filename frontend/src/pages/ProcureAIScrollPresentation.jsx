...
const ProcureAIScrollPresentation = () => {
  ...
  const handleDownloadPptx = async () => {
    ...
    try {
      ...
      pptx.author = "Procure AI";
      pptx.title = "Process Flowcharts & Database Architecture";
      pptx.subject = "IHS Towers Nigeria Pre-Alignment";
      
      // Slide 1: Title
      let slide1 = pptx.addSlide();
      slide1.background = { color: "1E2761" };
      slide1.addShape(pptx.ShapeType.rect, { x: 0, y: 0, w: "100%", h: 0.1, fill: { color: "0D9488" } });
      slide1.addText("Procure AI", { x: 0.5, y: 2, w: "90%", h: 1, fontSize: 48, color: "FFFFFF", fontFace: "Georgia", align: "center" });
      slide1.addText("Process Flowcharts & Back-End Database Architecture", { x: 0.5, y: 3, w: "90%", h: 0.5, fontSize: 20, color: "CADCFC", align: "center" });
      slide1.addText("IHS Towers Nigeria — Pre-Alignment Session", { x: 0.5, y: 4, w: "90%", h: 0.3, fontSize: 14, color: "94A3B8", align: "center" });
      slide1.addText("February 2026", { x: 0.5, y: 4.4, w: "90%", h: 0.3, fontSize: 12, color: "64748B", align: "center" });
      slide1.addText("CONFIDENTIAL", { x: 0.3, y: 5.2, w: 1.5, h: 0.2, fontSize: 10, color: "0D9488" });

      // Slide 2: Architecture
      let slide2 = pptx.addSlide();
      slide2.background = { color: "F8FAFC" };
      slide2.addText("High-Level System Architecture", { x: 0.5, y: 0.3, w: "90%", h: 0.5, fontSize: 28, color: "1E2761", fontFace: "Georgia" });
      slide2.addText("How Procure AI connects with IHS's existing infrastructure", { x: 0.5, y: 0.8, w: "90%", h: 0.3, fontSize: 14, color: "64748B" });
      
      // IHS Systems
      slide2.addText("IHS EXISTING SYSTEMS", { x: 0.3, y: 1.4, w: 2.5, h: 0.3, fontSize: 12, color: "FFFFFF", fill: { color: "1E2761" }, align: "center" });
      const ihsSystems = ["D365 ERP", "ServiceNow", "Azure Data Lake", "Lumen Contracts", "Active Directory"];
      ihsSystems.forEach((sys, i) => {
        slide2.addText(sys, { x: 0.3, y: 1.8 + (i * 0.5), w: 2.5, h: 0.4, fontSize: 11, color: "1E2761", fill: { color: "FFFFFF" }, line: { color: "E5E7EB" }, align: "center" });
      });

      // API Hub
      slide2.addText("API Integration Hub", { x: 3.5, y: 2.5, w: 2.5, h: 0.6, fontSize: 14, color: "FFFFFF", fill: { color: "0D9488" }, align: "center" });

      // Procure AI
      slide2.addText("PROCURE AI PLATFORM", { x: 6.5, y: 1.4, w: 3, h: 0.3, fontSize: 12, color: "FFFFFF", fill: { color: "0D9488" }, align: "center" });
      const modules = [["RFQ Engine", "059669"], ["Vendor Portal", "2563EB"], ["Auction System", "EA580C"], ["AI/ML Service", "1E2761"], ["Analytics", "0D9488"], ["Contract Mgmt", "64748B"]];
      modules.forEach((mod, i) => {
        const row = Math.floor(i / 2);
        const col = i % 2;
        slide2.addText(mod[0], { x: 6.5 + (col * 1.5), y: 1.8 + (row * 0.6), w: 1.4, h: 0.5, fontSize: 10, color: "FFFFFF", fill: { color: mod[1] }, align: "center" });
      });

      // Slide 3: RFQ Flow
      let slide3 = pptx.addSlide();
      slide3.background = { color: "F8FAFC" };
      slide3.addText("Process Flow: RFQ / Tender Creation", { x: 0.5, y: 0.3, w: "90%", h: 0.5, fontSize: 28, color: "1E2761", fontFace: "Georgia" });
      slide3.addText("End-to-end flow from requirement to vendor award", { x: 0.5, y: 0.8, w: "90%", h: 0.3, fontSize: 14, color: "64748B" });
      
      const rfqSteps = [
        ["1", "Business User Raises Request", "2563EB"],
        ["2", "Scope Validation", "0D9488"],
        ["3", "Budget & Approval Check", "EA580C"],
        ["4", "AI Vendor Discovery", "059669"],
        ["5", "RFQ Generation", "1E2761"],
        ["6", "Vendor Bid Submission", "2563EB"],
        ["7", "AI Bid Evaluation", "059669"],
        ["8", "Technical & Financial Scoring", "0D9488"],
        ["9", "BAFO Round (if required)", "EA580C"],
        ["10", "Award & Contract", "1E2761"],
      ];
      
      rfqSteps.forEach((step, i) => {
        const row = Math.floor(i / 5);
        const col = i % 5;
        slide3.addShape(pptx.ShapeType.ellipse, { x: 0.5 + (col * 1.9), y: 1.4 + (row * 1.8), w: 0.4, h: 0.4, fill: { color: step[2] } });
        slide3.addText(step[0], { x: 0.5 + (col * 1.9), y: 1.4 + (row * 1.8), w: 0.4, h: 0.4, fontSize: 10, color: "FFFFFF", align: "center", valign: "middle" });
        slide3.addText(step[1], { x: 0.3 + (col * 1.9), y: 1.9 + (row * 1.8), w: 1.8, h: 0.6, fontSize: 9, color: "1E2761", align: "center" });
      });

      // Slide 4: Vendor Onboarding
      let slide4 = pptx.addSlide();
      slide4.background = { color: "F8FAFC" };
      slide4.addText("Process Flow: Vendor Registration & Onboarding", { x: 0.5, y: 0.3, w: "90%", h: 0.5, fontSize: 28, color: "1E2761", fontFace: "Georgia" });
      slide4.addText("Self-service registration through to D365 sync", { x: 0.5, y: 0.8, w: "90%", h: 0.3, fontSize: 14, color: "64748B" });
      
      // Lane headers
      slide4.addText("VENDOR ACTIONS", { x: 0.3, y: 1.3, w: 3, h: 0.3, fontSize: 11, color: "FFFFFF", fill: { color: "2563EB" }, align: "center" });
      slide4.addText("PROCURE AI (AUTOMATED)", { x: 3.5, y: 1.3, w: 3, h: 0.3, fontSize: 11, color: "FFFFFF", fill: { color: "0D9488" }, align: "center" });
      slide4.addText("IHS PROCUREMENT", { x: 6.7, y: 1.3, w: 3, h: 0.3, fontSize: 11, color: "FFFFFF", fill: { color: "1E2761" }, align: "center" });

      // Slide 5: Reverse Auction
      let slide5 = pptx.addSlide();
      slide5.background = { color: "F8FAFC" };
      slide5.addText("Process Flow: Reverse Auction", { x: 0.5, y: 0.3, w: "90%", h: 0.5, fontSize: 28, color: "1E2761", fontFace: "Georgia" });
      slide5.addText("Asset disposal through competitive bidding", { x: 0.5, y: 0.8, w: "90%", h: 0.3, fontSize: 14, color: "64748B" });

      // Slide 6: Database Architecture
      let slide6 = pptx.addSlide();
      slide6.background = { color: "F8FAFC" };
      slide6.addText("Back-End Database Architecture", { x: 0.5, y: 0.3, w: "90%", h: 0.5, fontSize: 28, color: "1E2761", fontFace: "Georgia" });
      slide6.addText("Core data domains and storage strategy", { x: 0.5, y: 0.8, w: "90%", h: 0.3, fontSize: 14, color: "64748B" });
      
      const dbCards = [
        ["AZURE SQL DATABASE", "2563EB", "Transactional Data"],
        ["AZURE COSMOS DB", "0D9488", "Vendor Profiles & Documents"],
        ["AZURE DATA LAKE", "059669", "Analytics & ML Training"],
        ["AZURE BLOB STORAGE", "EA580C", "Files & Attachments"],
      ];
      dbCards.forEach((card, i) => {
        slide6.addText(card[0], { x: 0.3 + (i * 2.4), y: 1.4, w: 2.3, h: 0.4, fontSize: 10, color: "FFFFFF", fill: { color: card[1] }, align: "center" });
        slide6.addText(card[2], { x: 0.3 + (i * 2.4), y: 1.85, w: 2.3, h: 0.3, fontSize: 9, color: "64748B", align: "center" });
      });

      // Slide 7: Data Upload
      let slide7 = pptx.addSlide();
      slide7.background = { color: "F8FAFC" };
      slide7.addText("Bulk Data Upload: Format Requirements", { x: 0.5, y: 0.3, w: "90%", h: 0.5, fontSize: 28, color: "1E2761", fontFace: "Georgia" });
      slide7.addText("What IHS needs to prepare for initial data migration", { x: 0.5, y: 0.8, w: "90%", h: 0.3, fontSize: 14, color: "64748B" });

      // Slide 8: Next Steps
      let slide8 = pptx.addSlide();
      slide8.background = { color: "1E2761" };
      slide8.addShape(pptx.ShapeType.rect, { x: 0, y: 0, w: "100%", h: 0.1, fill: { color: "0D9488" } });
      slide8.addText("Next Steps", { x: 0.5, y: 0.5, w: "90%", h: 0.6, fontSize: 36, color: "FFFFFF", fontFace: "Georgia" });
      
      const nextSteps = [
        ["01", "IHS to extract D365 vendor master data", "Using provided CSV/XLSX templates"],
        ["02", "IHS to compile category taxonomy", "3-level hierarchy with CapEx/OpEx classification"],
        ["03", "IHS IT to confirm API access", "D365, ServiceNow, Azure Data Lake endpoints"],
        ["04", "Joint session to validate flowcharts", "Confirm process alignment with current operations"],
        ["05", "Procure AI team to deliver staging environment", "For test uploads and validation ahead of go-live"],
      ];
      
      nextSteps.forEach((step, i) => {
        slide8.addText(step[0], { x: 0.5, y: 1.5 + (i * 0.8), w: 0.6, h: 0.5, fontSize: 20, color: "0D9488" });
        slide8.addText(step[1], { x: 1.2, y: 1.5 + (i * 0.8), w: 5, h: 0.3, fontSize: 14, color: "FFFFFF", bold: true });
        slide8.addText(step[2], { x: 1.2, y: 1.8 + (i * 0.8), w: 5, h: 0.3, fontSize: 11, color: "94A3B8" });
      });
      
      slide8.addText("Target: All data templates populated by 28 February 2026", { x: 0.5, y: 5.2, w: "90%", h: 0.3, fontSize: 12, color: "0D9488", italic: true });

      await pptx.writeFile({ fileName: "Procure-AI-Presentation.pptx" });
    } catch (error) {
